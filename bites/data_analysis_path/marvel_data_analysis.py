...
def most_popular_characters(characters=characters, top=5):
    ...
    """
    most_pop_char = Counter()
    for character in characters:
        if character.appearances != '':
            most_pop_char[(character.pid, character.name)] += int(character.appearances)
    return [char[0][1] for char in most_pop_char.most_common(top)]
...
def max_and_min_years_new_characters(characters=characters):
    ...
    for character in characters:
        if character.year != '':
            birthday[character.year] += 1
    return (birthday.most_common()[0][0], birthday.most_common()[-1][0])
...
def get_percentage_female_characters(characters=characters):
    ...
    """
    total_with_sex_info = 0
    females = 0
    for character in characters:
        if character.sex != '':
            total_with_sex_info += 1
            if 'Female' in character.sex:
                females += 1
    return round(100 * float(females)/float(total_with_sex_info), 2)
