...
def make_character_index(text=text, characters=CHARACTERS):
    ...
    """
    characters_index = defaultdict(list)
    text_listed = text.splitlines()
    for id_line, line in enumerate(text_listed):
        for character in characters:
            if type(character) is not tuple and character.lower() in line.lower():
                characters_index[character.lower()].append(id_line)
            elif type(character) is tuple:
                for gm in character:
                    if gm.lower() in line.lower() and \
                        id_line not in characters_index[character[0].lower()]:
                        characters_index[character[0].lower()].append(id_line)
    return dict(characters_index)
...
if __name__ == "__main__":
    ...
    print('----')
    print(make_character_index(text=the_neetle_tree, characters=characters))